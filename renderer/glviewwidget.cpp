...
void glViewWidget::drawTrack(trackHandler *_track, bool toNormalMap)
{
    ...
    if (myTrack->drawMode != track::DRAW_MODE_HEARTLINE_ONLY && myTrack->lSections.size() > 0)
    {
    ...
    if (myTrack->drawMode != track::DRAW_MODE_TRACK_ONLY)
    {
...
void glViewWidget::drawSimpleSM(trackHandler *_track)
{
    ...
    if (myTrack->drawMode != track::DRAW_MODE_HEARTLINE_ONLY)
    {
        glBindVertexArray(mesh->TrackObject[0]);
        for(int i = 0; i < mesh->pipeBorders.size()-1; ++i)
        {
...
void glViewWidget::drawShadowVolumes()
{
    ...
    {
        if(trackList[i]->trackData->isDrawn)
        {
    ...
    {
        if (trackList[i]->trackData->isDrawn &&
            trackList[i]->trackData->drawMode != track::DRAW_MODE_HEARTLINE_ONLY &&
            trackList[i]->trackData->lSections.size() > 0)
        {
            trackMesh* mesh = trackList[i]->mMesh;
            track* myTrack = trackList[i]->trackData;
            anchorBase = glm::translate(myTrack->startPos) * glm::rotate(TO_RAD(myTrack->startYaw-90.f), glm::vec3(0.f, 1.f, 0.f));
...
void glViewWidget::drawOcclusion()
{
    ...
    for (int i = 0; i < trackList.size(); ++i)
    {
        if (trackList[i]->trackData->isDrawn)
        {
            ...
            if (!trackList[i]->mMesh->isWireframe)
            {
                drawTrack(trackList[i], true);
            }
        }
    }
    ...
    if(!legacyMode)
    {
        ...
        else // rift mode
        {
            ...
            for (int p = 0; p < 2; ++p)
            {
                ...
                if (shadowMode <= 0)
                {
                    simpleShadowFb->clear();
                }
                else
                {
                    shadowVolumeFb->clear();
                }
...
{
    if (_track->trackData->drawMode == track::DRAW_MODE_HEARTLINE_ONLY)
    {
        return;
    }
    ...
    if (_track->trackData->drawMode != track::DRAW_MODE_TRACK_ONLY)
    {
        for(int i = 0; i < myTrack->lSections.size(); i++)
        {
            section* curSection = myTrack->lSections[i];
